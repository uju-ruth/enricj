      ...
      <main className="max-w-6xl mx-auto px-6 pt-32 pb-16">
        <section className="grid md:grid-cols-2 gap-16 items-center min-h-[82vh]">
          <div className="fade-up">
            ...
            <h1
              className="font-serif font-light leading-[1.1] text-ink mb-5"
            ...
            <div className="rule" />
            <p
              className="font-serif text-stone leading-relaxed mb-10 max-w-md"
              style={{ fontSize: "1.22rem" }}
            >
              I explore the invisible systems shaping how we live, from
              economics and technology to human behavior.
            </p>
            <div className="flex flex-wrap gap-4">
              ...
              <Link to="/essays" className="btn-outline">
                Essays
              </Link>
            </div>
          </div>
      ...
      />
          ...
          {[
            {
              label: "Based in",
              heading: "Belgium",
              sub: "Pharmaceutical Industry",
            },
            {
              label: "Interests",
              heading: "Systems",
              sub: "Geopolitics · Economics · Institutions",
            },
            {
              label: "Writing",
              heading: "Essays",
              sub: "Ideas developed beyond the feed",
            },
        ...
        <section className="mt-20 fade-up delay-4">
          <div className="flex items-baseline justify-between mb-6">
            ...
            <Link
              to="/essays"
              className="text-sienna text-[0.7rem] tracking-widest uppercase hover:text-ink transition-colors"
            >
              View All →
            </Link>
          </div>
          {loading ? (
            <p className="text-stone text-sm italic border-t border-mist pt-6">
              Loading essays...
            </p>
          ) : (
            recent.map((e) => (
              <Link
                key={e.id}
                to={`/essays/${e.slug}`}
                className="block essay-card group"
              >
                <div className="flex justify-between items-start gap-4">
                  ...
                  {e.img && (
                    <img
                      src={e.img}
                      alt={e.title}
                      className="hidden md:block w-28 h-20 object-cover object-center flex-shrink-0"
                      style={{ filter: "grayscale(10%)" }}
                      onError={(ev) => (ev.target.style.display = "none")}
                    />
                  )}
                </div>
                <p className="text-sienna text-[0.7rem] tracking-widest uppercase mt-3">
                  Read Essay →
                </p>
              </Link>
            ))
